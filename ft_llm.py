...
def train(
    ...
    data_path: str = "data/nli_for_simcse_kor.csv",  # kornli로 변경
    ...
    if ddp:
        print(
            f"batch_size: {batch_size}, mirco_batch_size: {micro_batch_size}, world_size: {world_size}"
        )
    ...
    def tokenize(prompt, add_eos_token=True, label_prompt=None, neg_prompt=None):
        ...
        if (
            result["input_ids"][-1]
            != tokenizer.eos_token_id  # 마지막 토큰이 eos_token이 아니고
            and len(result["input_ids"])
            < cutoff_len  # 토큰의 길이가 cutoff_len보다 작고
            and add_eos_token  # eos_token 설정값이 True일 때
        ):
            result["input_ids"].append(tokenizer.eos_token_id)  # eos_token을 추가
            result["attention_mask"].append(1)  # attention_mask에 1 추가
        if label_prompt:  # label_prompt가 주어질 때
            ...
            result["labels"] = label_result[
                "input_ids"
            ]  # 토큰화 결과를 result["labels"]에 저장
            if neg_prompt:  # neg_prompt가 주어질 때
                neg_result = tokenizer(
                    neg_prompt,
                    padding=False,
                    return_tensors=None,
                )
                result["attention_mask"] = neg_result[
                    "input_ids"
                ]  # 토큰화 결과를 result["attention_mask"]에 저장
        else:  # label_prompt가 주어지지 않을 때
            result["labels"] = result[
                "input_ids"
            ].copy()  # input_ids를 복사하여 labels에 저장
    ...
    if load_kbit == 4:
        ...
        def find_all_linear_names(model):
            ...
            cls = bnb.nn.Linear4bit  # 찾고자 하는 모듈의 클래스 저장
            lora_module_names = set()  # 모듈 이름을 저장할 set 생성
            for (
                name,
                module,
            ) in model.named_modules():  # 모델의 모든 모듈을 순회하며 (모듈 이름, 모듈)
                if isinstance(module, cls):
                    names = name.split(".")
                    lora_module_names.add(
                        names[0] if len(names) == 1 else names[-1]
                    )  # 모듈 이름이 계층적으로 구성되어 있을 경우 마지막 이름을 저장하고 아닐 경우 첫 번째 이름을 저장

            if (
                "lm_head" in lora_module_names
            ):  # lm_head라는 모듈은 16비트가 필요하므로 삭제
        ...
        config = LoraConfig(
            r=lora_r,  # 랭크 (default: 64)
            lora_alpha=lora_alpha,  # 학습률 (default: 16)
            target_modules=target_modules,  # 타겟 모듈 (default: ['q_proj', 'v_proj']) # 로라가 적용되는 모듈의 이름
            lora_dropout=lora_dropout,  # 드롭아웃 (default: 0.05)
            bias="none",
            task_type="CAUSAL_LM",  # 텍스트의 다음 토큰을 예측하는 태스크 타입
        )
        ...
        for name, module in model.named_modules():
            ...
            if "lm_head" in name or "embed_tokens" in name:
                if hasattr(module, "weight"):  # module에 weight 속성이 있는 경우
                    # module = module.to(torch.bfloat16)
                    module = module.to(torch.float32)
    else:
        if load_kbit == 8:
